...
public class StateResource {
	...
	public Response getState(@QueryParam("countryId") @NotNull @Min(1)  int countryId) throws Exception {
		List<State> states;
		try {
			states = stateRepository.getStateByCountryId(countryId);

			/* If returned countryId doesn't return any values */
			if (states.isEmpty()) {
		...
		/* Any other kind of exception */
			...
			return Response.status(500).entity(new Message("Internal Server Error")).build();
		}

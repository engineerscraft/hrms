...
public class JobRoleResource {
    ...
    public Response getDistrict(@QueryParam("orgId") @Min(1) int orgId) {
        ...
        try {
            ...
            if (jobRoles.isEmpty()) {
                ...
                return Response.status(Response.Status.NOT_FOUND).entity(new Message("No job role is found.")).build();
            }
            /* If data presents in DB */
            else
                return Response.status(Response.Status.OK).entity(jobRoles).build();
        } catch (Exception e) {
    ...
    public Response getSalaryByJobRoleId(@PathParam("id") long jobRoleId) {
        ...
        try {
            ...
            if (salartList.isEmpty()) {
                ...
                return Response.status(Response.Status.NOT_FOUND).entity(new Message("No salary list is found.")).build();
            }
            /* If data presents in DB */
            else
                return Response.status(Response.Status.OK).entity(salartList).build();
        } catch (Exception e) {
    ...
    public Response getOptionalBenefitsByJobRoleId(@PathParam("id") long jobRoleId) {
        List<Salary> salartList;
        try {
            salartList = jobRoleRepository.getOptionalBenefitsByJobRoleId(jobRoleId);
            if (salartList.isEmpty()) {
                logger.error("No Optional Benefits list is found.");
                return Response.status(Response.Status.NOT_FOUND).entity(new Message("No Optional Benefits list is found.")).build();
            }
            /* If data presents in DB */
            else
                return Response.status(Response.Status.OK).entity(salartList).build();
